...
function apply(ctx, config) {
    ...
    ctx.command('activelink.admin.proactive', '强制触发一次主动消息（管理员）')
        .userFields(['authority'])
        .action(async ({ session }) => {
        if (!session || !session.user || session.user.authority < 4) {
            return '权限不足';
        ...
        const result = await proactiveTrigger.forceTriggerOnce();
        return `主动消息强制触发完成：成功 ${result.success} 个，失败 ${result.failed} 个`;
    });
}
